...
此代码的问题在于，在每个动画帧中，它都会先更改每个方块的样式（第70行），然后再查询页面上每个方块的位置。由于样式发生了变化，浏览器不会使用之前已经得知并缓存的样式数据信息，而需要重新布局并计算获取新布局方块的样式数据信息（称之为强制同步布局），以计算其位置所在。这就导致了性能损耗。
...
- 较多的动效影响性能。这是我们讲这些动效尽量放在一个单独的“层”里面，使用 transform 等属性，调用GPU渲染，并且减少gif图的使用
- 又有可能“图层”比较多，同样也影响性能
- 等等...
