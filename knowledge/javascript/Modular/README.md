...
在平常开发时，大家经常会写```require('xxxxxx');```或者```import xxx form 'xxx';```，他么你有什么不同呢？; 还有用过 webpack 或者 rollup.js 等工具的同学们也会对 amd、cjs、es、iife、umd 等词语比较耳熟，他们都代表上面含义呢？；打开antd的包，你会发现有dist,es和lib目录，这有时为什么呢？
